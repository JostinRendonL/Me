...
export const i18n = {
  en: {
    ...
    'hero.desc':        "Systems Engineering student who ships real software — SaaS apps in production, REST APIs, and AI-powered automation for Ecuadorian companies. I don't just learn — I deliver.",
    ...
    'proj.cat.saas':    'SaaS · Full-Stack',
    'proj.cat.backend': 'Backend · REST API',
    'proj.cat.ai':      'AI · Automation',
    'proj.cat.automation':'Automation · OCR',
    'proj.descV':       'Background-check SaaS that queries 4 official Ecuadorian government sources in parallel and returns a risk semaphore. Multi-user, PDFs with verifiable QR codes, LOPDP legal compliance, 100+ tests. In production for RUBASA.',
    'proj.descBG':      'REST API with concurrent scrapers that query 4 official government portals in ~10 seconds. Redis cache, automatic captcha solving, residential proxy rotation, deep health checks, and Prometheus metrics.',
    'proj.descCV':      'Hands-free CV-screening system with an AI fallback chain (GPT-4o → Gemini → Llama). Receives applications, analyzes the CV, cross-checks background records, and writes the report. Telegram alerts on critical cases.',
    'proj.descMA':      'Web app that automates biometric time-clock processing: reads the clock Excel, extracts manual records from photos with OCR, auto-assigns shifts, and computes payments. Generates HR-ready attendance reports.',
    'skill.backend':    'Backend & SaaS',
    ...
    'footer.copy':      '© 2026 Jostin Rendón. All rights reserved.',
  ...
  es: {
    ...
    'hero.desc':        "Estudiante de Ingeniería en Sistemas que entrega software real — apps SaaS en producción, APIs REST y automatización con IA para empresas ecuatorianas. No solo aprendo — entrego resultados.",
    ...
    'proj.cat.saas':    'SaaS · Full-Stack',
    'proj.cat.backend': 'Backend · API REST',
    'proj.cat.ai':      'IA · Automatización',
    'proj.cat.automation':'Automatización · OCR',
    'proj.descV':       'SaaS de verificación de antecedentes que consulta 4 fuentes oficiales del Estado ecuatoriano en paralelo y devuelve un semáforo de riesgo. Multi-usuario, PDFs con QR verificable, cumplimiento legal LOPDP, 100+ tests. En producción para RUBASA.',
    'proj.descBG':      'API REST con scrapers concurrentes que consultan 4 portales oficiales del Estado en ~10 segundos. Cache Redis, resolución automática de captchas, rotación de proxies residenciales, healthcheck profundo y métricas Prometheus.',
    'proj.descCV':      'Sistema hands-free de filtrado de hojas de vida con cadena de IAs y fallback automático (GPT-4o → Gemini → Llama). Recibe postulaciones, analiza el CV, cruza con verificación de antecedentes y escribe el reporte. Alerta por Telegram en casos críticos.',
    'proj.descMA':      'App web que automatiza el procesamiento de marcaciones biométricas: lee Excel del reloj, extrae registros manuales desde fotos con OCR, asigna turnos automáticamente y calcula pagos. Genera reportes de asistencia listos para RR.HH.',
    'skill.backend':    'Backend y SaaS',
    ...
    'footer.copy':      '© 2026 Jostin Rendón. Todos los derechos reservados.',
...
export function initTyping() {
  ...
  el.textContent = '';
  const words = typingWords[_currentLang] || typingWords.en;
  ...
  function tick() {
    ...
    typingTimer = setTimeout(tick, deleting ? 40 : 65);
  }
...
export function initLang() {
  ...
  document.querySelectorAll('.lang-toggle, .mobile-lang').forEach(wrap => {
    wrap.addEventListener('click', () => {
      switchLang(_currentLang === 'en' ? 'es' : 'en');
    });
  });
}
